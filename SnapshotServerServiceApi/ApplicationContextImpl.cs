...
using Snapshot.Server.Service.Core.Service;
using Snapshot.Server.Service.Core.Service.Service;
using Snapshot.Server.Service.Gateway;
...
[assembly : InternalsVisibleTo ("Snapshot.Server.Service.Api.Tests")]
namespace Foxpict.Service.Web {
  ...
  public class ApplicationContextImpl : IApplicationContext {
    ...
    /// <summary>
    /// アプリケーションに埋め込んでいるアセットファイルを参照するためのパス
    /// </summary>
    static readonly string APP_ASSETS_PATH = "SnapshotServerServiceApi.Assets";
    ...
    /// <summary>
    /// DIコンテナに、アプリケーションが使用するコンポーネントの登録を行います。
    /// </summary>
    ...
    public void SetDiContainer (SimpleInjector.Container container) {
      ...
      container.Register<IThumbnailBuilder, ThumbnailBuilderService> ();
    ...
    /// <summary>
    /// アプリケーションが使用するデータベースを作成します。
    /// </summary>
    private void CreateSettingSQLite () {
      SqliteConnectionStringBuilder builder_AppDb = new SqliteConnectionStringBuilder ();
      builder_AppDb.DataSource = Path.Combine (DatabaseDirectoryPath, "snapshot.db");
    ...
    private void InitializeAppDatabase () {
      ...
      if (isInitializeDatabase) {
        ...
        string filePath = string.Format (APP_ASSETS_PATH + ".Sql.{0}.Initialize_sql.txt", "App");
    ...
    private void InitializeThumbnailDatabase () {
      ...
      if (isInitializeDatabase) {
        ...
        using (var stream = assm.GetManifestResourceStream (string.Format (APP_ASSETS_PATH + ".Sql.{0}.Initialize_sql.txt", "Thumbnail"))) {
          using (StreamReader reader = new StreamReader (stream)) {
            sqltext = reader.ReadToEnd ();
          }
        }
    ...
    private string UpgradeFromResource (string dbselect, string version, KatalibDbContext @dbc) {
      ...
      var r = new Regex (string.Format (APP_ASSETS_PATH + ".Sql.{0}.{1}", dbselect, "upgrade - " + currentVersion + "-(.+)\\.txt"));
